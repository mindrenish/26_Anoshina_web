...
async function menu(type){
    switch (type){
        ...
        case "POST":
            // try {
            //     const response = await fetch("https://official-joke-api.appspot.com/random_joke", {method: "POST", type: "programming"});
            //     if (!response.ok){
            //         throw new Error('Error: ' + response.status)
            //     }
            //     const json = await response.json();
